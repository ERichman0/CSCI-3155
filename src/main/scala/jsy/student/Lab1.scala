...
object Lab1 extends jsy.util.JsyApplication with jsy.lab1.Lab1Like {
  ...
  def xor(a: Boolean, b: Boolean): Boolean = {
    val a_int = if (a) 1 else 0
    val b_int = if (b) 1 else 0
    if (a_int + b_int == 1) true else false
  }

  def repeat(s: String, n: Int): String = {
    require(n >= 0)
    //if (n==0) ("") else (repeat(s,n-1) + s)
    n match {
      case 0 => ""
      case _ => repeat(s,(n-1))+s
    }
  }
  ...
  def sqrtN(c: Double, x0: Double, n: Int): Double = {
    require(n >= 0)
    if (n == 0) x0
    else sqrtStep(c, sqrtN(c, x0, n-1))
  }
  ...
  def repOk(t: SearchTree): Boolean = {
    def check(t: SearchTree, min: Int, max: Int): Boolean = t match {
      case Empty => true
      case Node(l, d, r) => {
        if((d < max)&&(d >= min) && check(l, min, d) && check(r, d, max)){true}
  ...
  def insert(t: SearchTree, n: Int): SearchTree = t match {
    ...
    case Node(l, d, r) => {
      if(n<d) Node(insert(l, n), d, r) //Checks value inserted vs current node, value and recurse through left or right child
  ...
  def deleteMin(t: SearchTree): (SearchTree, Int) = {
    ...
    (t: @unchecked) match {
      ...
      case Node(l, d, r) =>          //recurse if left node exists 
        val (l1, m) = deleteMin(l) //left node delete min
  ...
  def delete(t: SearchTree, n: Int): SearchTree = {
    t match{
      case Empty => Empty
      case Node(l, d, r) =>  //Check n on current node != recurse on child
        if(n<d) Node(delete(l,n),d,r)
        else if(n>d) Node(l,d, delete(r,n))
        else if(l == Empty) r //Replace current node with right child
        else if(r == Empty) l //Replace current node with left child
        else{
          val temp = deleteMin(r) //Replace current node with the min node of the right child
          Node(l, temp._2, temp._1) //Build node using ._(accessing collections)
        }
    }
  }
